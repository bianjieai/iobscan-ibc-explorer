...
import { getIbcStatistics, getIbcChains, getIbcTxs, getIbcDenoms, getIbcBaseDenoms } from '../service/api';
import {
    GET_IBCSTATISTICS,
    GET_IBCCHAINS,
    GET_IBCTXS,
...
export const useIbcStatisticsChains = defineStore('global', {
    state: () => {
        return {
            ...
            ibcChains: JSON.parse(
                sessionStorage.getItem('allChains')?.length ? sessionStorage.getItem('allChains') : '{}'
            ),
            ...
            ibcBaseDenoms: JSON.parse(
                sessionStorage.getItem('ibcBaseDenoms')?.length
                    ? sessionStorage.getItem('ibcBaseDenoms')
                    : '{"value":[]}'
            ),
                ...
                if (use_count) {
                    ...
                    const getSymbolInfo = (oldData) => {
                        ...
                        return result.map((item, index) => {
                            ...
                            let symbolIcon = '';
                            if (symbol) {
                                const findSymbol = Tools.findSymbol(this.ibcBaseDenoms.value, symbol);
                                if (findSymbol) {
